...
export function SavePropertyButton({ propertyId, initiallySaved, iconOnly = false }: { propertyId: string, userId?: string, initiallySaved: boolean, iconOnly?: boolean }) {
  ...
  const handleSave = (e: React.MouseEvent) => {
    ...
    startTransition(async () => {
      try {
        const isSavedNow = await toggleSavedLodge(propertyId);
        setSaved(isSavedNow);
      } catch (error) {
        // Revert on error
