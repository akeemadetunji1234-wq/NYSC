...
export default function NotificationsPage() {
  ...
  const handleToggleSavedSearch = async (search: any, field: "active" | "emailAlerts") => {
    const nextValue = !search[field];
    setSavedSearches((prev) => prev.map((item) => item.id === search.id ? { ...item, [field]: nextValue } : item));
    try {
      await updateSavedSearch(search.id, { [field]: nextValue });
    } catch {
      setSavedSearches((prev) => prev.map((item) => item.id === search.id ? { ...item, [field]: search[field] } : item));
    }
  };
  ...
  return (
    <div className="min-h-screen bg-secondary">
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 md:py-10">
        ...
        {savedSearches.length > 0 && <div className="bg-card rounded-2xl border border-border p-4 mb-6"><h2 className="font-bold text-foreground mb-3">Your saved searches</h2><div className="space-y-3">{savedSearches.map((search) => <div key={search.id} className="rounded-xl bg-secondary px-3 py-3"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold text-foreground">{search.name}</p><p className="text-xs text-muted-foreground">{[search.state, search.lga, search.minPrice && `from ₦${search.minPrice.toLocaleString()}`, search.maxPrice && `up to ₦${search.maxPrice.toLocaleString()}`, search.bedrooms && `${search.bedrooms}+ bedrooms`].filter(Boolean).join(" · ") || "Any published property"}</p></div><button onClick={() => deleteSavedSearch(search.id).then(refresh)} className="text-muted-foreground hover:text-red-600" aria-label={`Delete ${search.name}`}><Trash2 className="w-4 h-4" /></button></div><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => handleToggleSavedSearch(search, "active")} className={`rounded-lg border px-2.5 py-1.5 text-xs font-semibold ${search.active ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-border bg-background text-muted-foreground"}`}>{search.active ? "Search active" : "Search paused"}</button><button type="button" onClick={() => handleToggleSavedSearch(search, "emailAlerts")} className={`rounded-lg border px-2.5 py-1.5 text-xs font-semibold ${search.emailAlerts ? "border-sky-200 bg-sky-50 text-sky-800" : "border-border bg-background text-muted-foreground"}`}>{search.emailAlerts ? "Email alerts on" : "Email alerts off"}</button></div></div>)}</div></div>}
