...
| P1 | Capture deployed Core Web Vitals and route-specific resource timing for `/`, `/signin`, `/member`, `/agent`, and `/control-room-7f3k9d`. | Local TTFB cannot expose CDN, browser, serverless cold-start, or real-device costs. | Compare LCP, INP, CLS, hydration time, JS transfer, image transfer, and database/API timing by route. |
